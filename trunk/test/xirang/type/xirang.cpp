...
BOOST_AUTO_TEST_CASE(xirang_setup_case)
{
    ...
    BOOST_CHECK(xi.root().locateNamespace(".sys.type", '.').findType("int32").valid());
	...
	BOOST_CHECK(xi.root().findType("int") == xi.root().locateNamespace(".sys.type", '.').findType("int32"));

	Type tint = xi.root().findType("int");

	BOOST_CHECK(tint.fullName() == literal(".sys.type.int32"));
}
