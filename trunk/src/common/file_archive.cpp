...
namespace io{
	...
	range<file_read_archive::iterator> file_read_archive::read(const range<file_read_archive::iterator>& buf)
	{
		return m_imp->read(buf);
	}
	bool file_read_archive::readable() const { return m_imp->readable();}
	unique_ptr<read_view> file_read_archive::view_rd(ext_heap::handle h) const { return m_imp->view_rd(h);}
	...
	range<file_write_archive::const_iterator> file_write_archive::write(
			const range<file_write_archive::const_iterator>& r)
	{ return m_imp->write(r);}
	long_size_t file_write_archive::truncate(long_size_t size)	{ return m_imp->truncate(size);}
	bool file_write_archive::writable() const	{ return m_imp->writable(); }
	void file_write_archive::sync() 	{ m_imp->sync(); }
	unique_ptr<write_view> file_write_archive::view_wr(ext_heap::handle h){ return m_imp->view_wr(h);}
	...
	range<file_read_write_archive::iterator> file_read_write_archive::read(
			const range<file_read_write_archive::iterator>& buf)
	{	return m_imp->read(buf);	}

	bool file_read_write_archive::readable() const	{ return m_imp->readable(); }
	unique_ptr<read_view> file_read_write_archive::view_rd(ext_heap::handle h) const { return m_imp->view_rd(h);}
	range<file_read_write_archive::const_iterator> file_read_write_archive::write(
			const range<file_read_write_archive::const_iterator>& r)
	{ return m_imp->write(r); }
	...
	bool file_read_write_archive::writable() const	{ return m_imp->writable(); }
	void file_read_write_archive::sync() { m_imp->sync();}
	unique_ptr<write_view> file_read_write_archive::view_wr(ext_heap::handle h){ return m_imp->view_wr(h);}
